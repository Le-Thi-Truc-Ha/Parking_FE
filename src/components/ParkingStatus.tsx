...
const ParkingStatus = (): JSX.Element => {
    ...
    useEffect(() => {
        const ws = new WebSocket(`ws://${import.meta.env.VITE_ESP32_IP}:5001`);
        ...
        ws.onmessage = (event) => {
            try {
                ...
                if (msg.type === "ir") {
                    setIrData(msg.data.map((item: any) => (item.status)));
                }
            } catch (e) {
        ...
        <>
            <Row>
                <Col span={24} style={{paddingBottom: "20px"}}>
                    <div>{`Số vị trí trống: ${irData.filter((item) => (item == 1)).length}`}</div>
                </Col>
                <Col span={24}>
